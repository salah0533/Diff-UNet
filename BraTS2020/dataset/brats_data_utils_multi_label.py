...
def get_loader_brats(data_dir, batch_size=1, fold=0, num_workers=8):
    ...
    train_and_test_ids = pathListIntoIds(all_paths); 
    ...
    for id in train_and_test_ids:
        if id not in train_test_split_brats20['test']:
            train_test_split_brats20['train'].append(id)

    train_files = [os.path.join(data_dir, d) for d in train_test_split_brats20['train']][:1]
    test_files = [os.path.join(data_dir, d) for d in train_test_split_brats20['test']][:1]
    val_files = [os.path.join(data_dir, d) for d in train_test_split_brats20['test']][:1]
